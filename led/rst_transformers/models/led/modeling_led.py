...
from .DMRST.RST import *
from .DMRST.model_depth import ParsingNet
summarizer_tokenizer = AutoTokenizer.from_pretrained("allenai/led-large-16384", use_fast=True)
...
model_path = "parser.torchsave"
parser_backbone = parser_backbone.cuda()
for name, param in parser_backbone.named_parameters():
    param.requires_grad = False
Parser = ParsingNet(parser_backbone, bert_tokenizer=parser_tokenizer)
Parser = Parser.cuda()
Parser.load_state_dict(torch.load(model_path))
Parser = Parser.eval()
...
class LEDEncoder(LEDPreTrainedModel):
    ...
    def forward(
        ...
        max_length = 1024
        segments = split_text(Raw_sentence[0], max_length)
        demension = 1024
        document_weight_matrix = []
        for i in range(len(segments)):
            weight_matrix = generate_token_weight_matrix(Parser, parser_tokenizer, summarizer_tokenizer, [segments[i]], demension)
            document_weight_matrix.append(weight_matrix)
        # print(len(document_weight_matrix))
        document_rst_weight = torch.cat([matrix for matrix in document_weight_matrix], dim=0)
        del document_weight_matrix
        ...
        # 重点
        # 处理 n > m 的情况，丢弃多余的行
        if document_rst_weight.size(0) > inputs_embeds.size(1):
            document_rst_weight = document_rst_weight[:inputs_embeds.size(1), :]
        # 处理 n < m 的情况，使用广播自动扩展维度
        elif document_rst_weight.size(0) < inputs_embeds.size(1):
            padding = torch.zeros((inputs_embeds.size(1) - document_rst_weight.size(0), demension), device=document_rst_weight.device, dtype=document_rst_weight.dtype)
            document_rst_weight = torch.cat([document_rst_weight, padding], dim=0)
        document_rst_weight = document_rst_weight.unsqueeze(0)
        # 获取 hidden_states 所在的 GPU 设备
        hidden_states_device = inputs_embeds.device
        hidden_states_dtype = inputs_embeds.dtype
        # 将 document_rst_weight 移动到 hidden_states_device
        with torch.cuda.device(hidden_states_device):
            document_rst_weight = document_rst_weight.to(hidden_states_dtype).cuda()
        # 逐元素相乘
        document_rst_weight += torch.ones_like(document_rst_weight)
        inputs_embeds = document_rst_weight * inputs_embeds
